...
class WebSocket(HTTPConnection):
    # WebSocket 연결이 생성될 때 호출 (client_state와 application_state를 CONNECTING 상태로 초기화)
    ...
    async def receive(self) -> Message:
        ...
        - 클라이언트의 WebSocket 메시지를 비동기로 수신하며, 상태 전환을 관리
        """
        if self.client_state == WebSocketState.CONNECTING:
            ...
            # websocket.disconnect 타입의 메시지를 수신하면, client_state를 DISCONNECTED로 전환
    ...
    async def send(self, message: Message) -> None:
        ...
        - 서버에서 클라이언트로 메시지를 보낼 때 사용되며, 상태가 DISCONNECTED가 될 때까지 작동
        """
    ...
    async def accept(
        ...
        """
        1. 클라이언트의 연결 요청이 들어오면 서버는 websocket.connect 메시지를 수신하고, 
           receive() 메서드를 통해 이를 확인한 뒤 websocket.accept 메시지를 전송하여 연결을 수락
        """
        headers = headers or []
    ...
    # 메시지 타입이 websocket.disconnect인지 확인
    ...
    async def receive_text(self) -> str:
        if self.application_state != WebSocketState.CONNECTED:
            # websocket.disconnect 메시지를 감지하면 WebSocketDisconnect 예외를 발생시킴
